 ...
 */
/*
 * Boot redirect — called from main() after basic SDK init.
 * If ZERO_BLOCK invalid → returns to kickstarter (normal boot).
 * If ZERO_BLOCK valid → jumps to firmware's original Reset handler.
 *
 * Escape: not implemented here — hold BOOTSEL during power-on for USB recovery,
 * or use NES gamepad SELECT to skip (checked in main before calling this).
 */
__attribute__((section(".boot_redirect"), used))
int _boot_redirect_check(void) {
    if (ZB_XIP_ADDR[1023] != FLASH_MAGIC_OVER) {
        return 0;  /* No valid firmware — run kickstarter */
    }
