...
#
# release.sh - Build all release variants of frank-kickstart
#
# Creates firmware files for each board variant (M1, M2):
#   - UF2 format for direct flashing
#
# Output format: frank-kickstart_<board>_<version>.uf2
#
# Release configuration:
#   - CPU: 400 MHz (DVI bit clock)
#   - PSRAM: 133 MHz
...
echo ""
echo -e "${CYAN}┌─────────────────────────────────────────────────────────────────┐${NC}"
echo -e "${CYAN}│                frank-kickstart Release Builder                  │${NC}"
echo -e "${CYAN}└─────────────────────────────────────────────────────────────────┘${NC}"
echo ""
...
build_variant() {
    ...
    local OUTPUT_NAME="frank-kickstart_${BOARD_LC}_${VERSION}.uf2"
...
echo ""
ls -la "$RELEASE_DIR"/frank-kickstart_*_${VERSION}.uf2 2>/dev/null | awk '{print "  " $9 " (" $5 " bytes)"}'
echo ""
